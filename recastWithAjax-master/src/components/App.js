...
class App extends React.Component {
  ...
  searchVideo(queryString) {
    ...
    const url = `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${queryString}&key=AIzaSyB1SXLRNalAadsIW9ILMeijdt-jVODm1d8`;
    fetch(url)
      .then((data) => data.json())
      .then((json) => {
        this.setState({ videos: json.items, isLoading: false });
      });
    // setTimeout(() => {
    //   this.setState({
    //     isLoading: false,
